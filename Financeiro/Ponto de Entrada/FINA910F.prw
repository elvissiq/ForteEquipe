...
Local cCtaCDig := PADR(Substr(cConta, 1, nTamCC-1),TamSX3("E1_CONTA")[1])
Local cCtaSDig := PADR(Substr(cConta, 1, At('-',cConta)-1),TamSX3("E1_CONTA")[1])
...
Do Case 
    Case SA6->(MsSeek(FWxFilial("SA6")+cBanco+cAgencia+cConta))
        ...
        aAdd(aRet,cAgencia ) 
        aAdd(aRet,cConta   ) 
    Case SA6->(MsSeek(FWxFilial("SA6")+cBanco+cAgencia+cCtaCDig)) 
        aAdd(aRet,cBanco   )
        aAdd(aRet,cAgencia )
        aAdd(aRet,cCtaCDig )
    Case SA6->(MsSeek(FWxFilial("SA6")+cBanco+cAgencia+cCtaSDig))
        aAdd(aRet,cBanco   ) 
        aAdd(aRet,cAgencia ) 
        aAdd(aRet,cCtaSDig )
    OTHERWISE
