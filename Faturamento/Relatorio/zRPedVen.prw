...
Static Function fMontaRel(oProc)
	...
	IF ! (SC5->C5_TIPO $('D/B') )
	cQryPed += "    ISNULL(A1_NOME, '') AS NOME, "		       	 + CRLF
	cQryPed += "    ISNULL(A1_NREDUZ, '') AS NREDUZ, "      	 + CRLF
	cQryPed += "    ISNULL(A1_PESSOA, '') AS PESSOA, "        	 + CRLF
	cQryPed += "    ISNULL(A1_CGC, '') AS CGC, "              	 + CRLF
	cQryPed += "    ISNULL(A1_END, '') AS ENDERECO, "          	 + CRLF
	cQryPed += "    ISNULL(A1_BAIRRO, '') AS BAIRRO, "       	 + CRLF
	cQryPed += "    ISNULL(A1_MUN, '') AS MUNICIPIO, "         	 + CRLF
	cQryPed += "    ISNULL(A1_EST, '') AS ESTADO, "            	 + CRLF
	cQryPed += "    ISNULL(A1_DDD, '') AS DDD, "       			 + CRLF
	cQryPed += "    ISNULL(A1_TEL, '') AS TELEFONE, "  			 + CRLF
	cQryPed += "    ISNULL(A1_EMAIL, '') AS EMAIL, "       		 + CRLF
	Else 
	cQryPed += "    ISNULL(A2_NOME, '') AS NOME, "		       	 + CRLF
	cQryPed += "    ISNULL(A2_NREDUZ, '') AS NREDUZ, "      	 + CRLF
	cQryPed += "    ISNULL(A2_TIPO, '') AS PESSOA, "        	 + CRLF
	cQryPed += "    ISNULL(A2_CGC, '') AS CGC, "              	 + CRLF
	cQryPed += "    ISNULL(A2_END, '') AS ENDERECO, "          	 + CRLF
	cQryPed += "    ISNULL(A2_BAIRRO, '') AS BAIRRO, "       	 + CRLF
	cQryPed += "    ISNULL(A2_MUN, '') AS MUNICIPIO, "         	 + CRLF
	cQryPed += "    ISNULL(A2_EST, '') AS ESTADO, "            	 + CRLF
	cQryPed += "    ISNULL(A2_DDD, '') AS DDD, "       			 + CRLF
	cQryPed += "    ISNULL(A2_TEL, '') AS TELEFONE, "  			 + CRLF
	cQryPed += "    ISNULL(A2_EMAIL, '') AS EMAIL, "       		 + CRLF
	EndIF
	...
	cQryPed += "        AND A2_COD  = SC5.C5_CLIENTE "           + CRLF
	cQryPed += "        AND A2_LOJA = SC5.C5_LOJACLI "           + CRLF
...
Static Function fImpCab()
	...
	Local lCNPJ       := (QRY_PED->PESSOA != "F")
	Local cCliAux     := QRY_PED->C5_CLIENTE+" "+QRY_PED->C5_LOJACLI+" - "+QRY_PED->NOME
	Local cCGC        := ""
	Local cFretePed   := ""
	//Dados da empresa
	...
	oPrintPvt:SayAlign(nLinCab,    nColMeio+68, QRY_PED->NREDUZ,     	                        oFontCab, 300, 07, , nPadLeft, )
	nLinCab += 10
	cCGC := QRY_PED->CGC
	If lCNPJ
	...
	oPrintPvt:SayAlign(nLinCab, nColMeio+045,"("+QRY_PED->DDD+")",								oFontCab,  039, 07, , nPadLeft, )
	oPrintPvt:SayAlign(nLinCab, nColMeio+065,QRY_PED->TELEFONE,	 								oFontCab,  190, 07, , nPadLeft, )
	nLinCab += 10
	oPrintPvt:SayAlign(nLinCab, nColMeio+8, "E-mail:",		                                    oFontCabN, 030, 07, , nPadLeft, )
	oPrintPvt:SayAlign(nLinCab, nColMeio+038, QRY_PED->EMAIL,			 						oFontCab,  300, 07, , nPadLeft, )
	nLinCab += 10
...
Static Function fImpTot()
	...
	oPrintPvt:SayAlign(nLinAtu, nColIni+0095, Alltrim(Transform(nDescItem + nDescTot, cMaskVlr)),	oFontCabN, 080, 07, , nPadRight, )
